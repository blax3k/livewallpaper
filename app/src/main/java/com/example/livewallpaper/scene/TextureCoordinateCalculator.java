...
public class TextureCoordinateCalculator {
    ...
    /**
     * Extract texture scale and offset from current texture coordinates.
     * Reverses the math from updateTextureCoordinates to recover the original editing parameters.
     *
     * @param currentTexCoordinates the current texture coordinates (8 floats)
     * @param width current sprite width
     * @param height current sprite height
     * @param originalWidth original sprite width
     * @param originalHeight original sprite height
     * @param textureScaleFactor texture scale factor in world space
     * @return array of [textureScale, textureOffsetU, textureOffsetV]
     */
    public static float[] extractTextureEditState(
            float[] currentTexCoordinates,
            float width,
            float height,
            float originalWidth,
            float originalHeight,
        ...
        if (currentTexCoordinates == null || currentTexCoordinates.length < 8) {
            // Return default state if coordinates are invalid
            return new float[] { 1.0f, 0.0f, 0.0f };
        }

        // Extract the visible window bounds from texture coordinates
        // currentTexCoords layout: [TL_u, TL_v, BL_u, BL_v, TR_u, TR_v, BR_u, BR_v]
        float uMin = currentTexCoordinates[0];  // top-left U
        float uMax = currentTexCoordinates[4];  // top-right U
        float vMin = currentTexCoordinates[3];  // bottom-left V
        float vMax = currentTexCoordinates[1];  // top-left V

        // Calculate the visible window size
        float windowSizeU = uMax - uMin;
        float windowSizeV = vMax - vMin;

        // Calculate the growth scale of the sprite
        float widthGrowth = width / originalWidth;
        float heightGrowth = height / originalHeight;
        float growthScale = Math.max(widthGrowth, heightGrowth);

        // Calculate how much of the texture is visible
        float textureWidthInWorld = originalWidth * textureScaleFactor * growthScale;
        float textureHeightInWorld = originalHeight * textureScaleFactor * growthScale;
        float uScale = width / textureWidthInWorld;
        float vScale = height / textureHeightInWorld;

        // Calculate texture scale from window size
        // windowSize = (1.0 / textureScale) * scale, so textureScale = scale / windowSize
        float baseWindowSizeU = windowSizeU / uScale;
        float baseWindowSizeV = windowSizeV / vScale;
        float textureScale = 1.0f / Math.max(baseWindowSizeU, baseWindowSizeV);

        // Calculate offset from window position (center is 0.5, 0.5)
        float centerU = 0.5f;
        float centerV = 0.5f;
        float textureOffsetU = uMin - (centerU - (windowSizeU * 0.5f));
        float textureOffsetV = vMin - (centerV - (windowSizeV * 0.5f));

        return new float[] { textureScale, textureOffsetU, textureOffsetV };
    }
}
